...
def view_technologies():
    cur = db.cursor()
    cur.execute("SELECT * FROM technologies ORDER BY id ASC")
    techs = cur.fetchall()
...
def take_quiz(user_id):
    ...
    cur.execute(
        "INSERT INTO scores (user_id, technology_id, score, total, time, duration) VALUES (%s,%s,%s,%s,%s,%s)",
        (user_id, tech_id, score, total, end_time, duration)
    )
    db.commit()
